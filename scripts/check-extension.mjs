...
const hosts = [...new Set(adapterManifest.adapters.flatMap(adapter => adapter.connect || []))].sort();
...
requireValue(!background.includes('__ALLOWED_DOWNLOAD_HOSTS__'), 'Background contains an unreplaced host placeholder.');
requireValue(background.includes(JSON.stringify(hosts)), 'Background runtime allowlist differs from adapter hosts.');
...
console.log(`${target} extension ${packageJson.version} passed validation with ${hosts.length} generated download host permission(s).`);
